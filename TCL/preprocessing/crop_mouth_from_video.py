...
def load_args(default_config=None):
    ...
    # -- utils
    parser.add_argument('--video-direc', default='../datasets/lipread_mp4', help='raw video directory') #
    parser.add_argument('--landmark-direc', default='../landmarks/LRW_landmarks', help='landmark directory')
    parser.add_argument('--filename-path', default='./lrw500_detected_face.csv', help='list of detected video and its subject ID')
    parser.add_argument('--save-direc', default='../datasets/LRW_visual_new/', help='the directory of saving mouth ROIs') #
    # -- mean face utils
    ...
    # -- test set only
    parser.add_argument('--testset-only', default=True, action='store_true', help='process testing set only')
...
# lines = list(filter(lambda x: ('AB' == x.split('/')[0][0:2]) or ('AC' == x.split('/')[0][0:2]) or ('AFFAIRS' == x.split('/')[0]), lines)) if args.testset_only else lines
print(lines[-10:-1])
