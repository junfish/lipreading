...
def load_args(default_config=None):
    ...
    parser.add_argument('--annonation-direc', default='./datasets/lipread_mp4', help='Loaded data directory') #
    # -- model config
    ...
    # -- test
    parser.add_argument('--model-path', type=str, default='./models/lrw_resnet18_mstcn.pth.tar', help='Pretrained model pathname')
...
def evaluate(model, dset_loader, criterion):
    ...
    with torch.no_grad():
        for batch_idx, (input, lengths, labels) in enumerate(tqdm(dset_loader)): # (input, lengths, labels)
